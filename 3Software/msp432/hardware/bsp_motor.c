...
void Motor_Init(void)
{
	...
	MAP_GPIO_setAsPeripheralModuleFunctionOutputPin(GPIO_PORT_P8, GPIO_PIN0, GPIO_PRIMARY_MODULE_FUNCTION);//B
	...
	MAP_Timer_A_generatePWM(TIMER_A1_BASE, &TimA1_PWMConfig);
	TimA1_PWMConfig.compareRegister = TIMER_A_CAPTURECOMPARE_REGISTER_0;
	MAP_Timer_A_generatePWM(TIMER_A1_BASE, &TimA1_PWMConfig);
}
/**
 * @brief	电机控制函数
...
void Motor_Control(uint8_t Motorx,uint8_t Dir,uint8_t PWM)
{
	...
	switch(Motorx)
	{
		...
		case 2:	Motor_Port1	=	GPIO_PORT_P9;
				...
				Motor_PWM_Select = TIMER_A_CAPTURECOMPARE_REGISTER_0;
		break;
		case 3:
	...
	switch(PWM)
	{
		case 0 ... 10:Timer_A_setCompareValue(TIMER_A1_BASE,Motor_PWM_Select,PWM);
		break;
		case 11 ... 99:Timer_A_setCompareValue(TIMER_A1_BASE,Motor_PWM_Select,PWM);
		break;
	}	
}
